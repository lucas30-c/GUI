"""
DSL v0.1 JSON Schema 导出工具

提供确定性的 JSON Schema 导出功能：
- export_json_schema() 返回格式化 JSON 字符串
...
# 默认导出路径（相对于项目根目录）
_DEFAULT_OUTPUT_PATH = Path("contracts/dsl/v0.1/schema.json")


def export_json_schema() -> str:
    """
    导出 DslDocument 的 JSON Schema。

    返回确定性格式化的 JSON 字符串（sort_keys=True, indent=2），
    每次调用输出完全相同。含版本标记 x-dsl-version。
    """
    schema = DslDocument.model_json_schema()
    # FR-13: 添加 DSL 版本标记
    schema["x-dsl-version"] = "0.1"
    return json.dumps(schema, sort_keys=True, indent=2, ensure_ascii=False)


def _find_project_root() -> Path:
    """从当前文件向上查找项目根目录（含 backend/ 目录的父级）"""
    # 本文件位于 backend/src/genui_api/contracts/schema_export.py
    # 项目根目录是 backend 的父目录
    current = Path(__file__).resolve()
    # 向上 4 级：contracts -> genui_api -> src -> backend -> 项目根
...
def main() -> None:
    ...
    print(f"JSON Schema 已导出到: {output_path}")
